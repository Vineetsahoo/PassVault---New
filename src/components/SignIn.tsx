  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (email === 'user@example.com' && password === 'password') {
        // Store auth state in localStorage with expiration
        const expiresAt = Date.now() + 24 * 60 * 60 * 1000; // 24 hours from now
        
        // Use consistent authentication keys that Dashboard checks for
        localStorage.setItem('mockAuth', 'true');
        localStorage.setItem('isAuthenticated', 'true'); 
        localStorage.setItem('userToken', 'demo-token-12345');
        
        // Store user data in consistent formats
        const userData = { email, name: 'Demo User', expiresAt };
                ...
                <div className="mt-6 pt-3 border-t border-slate-200">
                  <div className="relative">
                    ...
                    <div className="relative flex justify-center text-xs uppercase">
                      <span className="bg-white px-3 py-1 rounded-full text-slate-500 text-xs font-medium">Demo Credentials</span>
                    </div>
                  </div>
                  <div className="mt-4 p-3 bg-violet-50/50 rounded-lg text-center text-sm border border-violet-100">
                    <p className="font-medium text-violet-800">Email: <span className="font-normal text-violet-700">user@example.com</span></p>
                    <p className="font-medium text-violet-800">Password: <span className="font-normal text-violet-700">password</span></p>
                  </div>
                </div>
              </div>
            </div>
